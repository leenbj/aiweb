    ...
    (set, get) => ({
      ...
      login: async (email: string, password: string) => {
        try {
          ...
          const { user, token } = response.data?.data || {} as { user?: SharedUser; token?: string };
          if (!user || !token) throw new Error('登录响应无效');
          
          set({
            user: { ...user, plan: (user as any).plan || 'free', role: (user as any).role || 'user' } as User,
            token,
      ...
      register: async (name: string, email: string, password: string) => {
        try {
          ...
          const { user, token } = response.data?.data || {} as { user?: SharedUser; token?: string };
          if (!user || !token) throw new Error('注册响应无效');
